...
/**
 * @file
 * Analyseur syntaxique Dataset57
 *
 * Format supporté:
 * @verbatim
...
static int parser_UNV57_read_header(io_block_t *block,
				    io_file_t *file,
				    io_value_t *arg);
static int parser_UNV57_read_header_only(io_block_t *block,
					 io_file_t *file,
					 io_value_t *arg);
static int parser_UNV57_read_all(io_block_t *block,
				 io_file_t *file,
				 io_value_t *arg);
static int parser_UNV57_write_header(io_block_t *block,
				     io_file_t *file,
				     io_value_t *arg);
static int parser_UNV57_write(io_block_t *block,
			      io_file_t *file,
			      io_value_t *arg);
...
const io_parser_function_t io_parser_UNV57[] = {
	{
		"read_header",
		parser_UNV57_read_header_only,
		IO_FILE_MODE_READ
	},
	{
		"read",
		parser_UNV57_read_all,
		IO_FILE_MODE_READ
	},
	{
		"mark",
...
io_value_t *arg)
{
	parser_UNV57_read_header(block, file, arg);
 ...
 */
#if 0
static int parser_UNV57_read_values(io_analysis_value_t *value,
				    io_file_t *file)
{
	const char *line;
	unsigned long j;
	...
	{		
		...
		/* On decode les valeurs resultats sur la ligne boucle j */
		line = io_decode_double(line, 13, value->values + j);
				 ...
				 io_value_t *arg)
{
	...
	/* Lecture de l'entete */
	status = parser_UNV57_read_header(block, file, arg);
	if(status != 0)
		return(-1);

	/* Lecture des valeurs resultats */
	if (io_block_get_long(block, "number_data_values", &number_data_values))
		return(-1);
	...
	while(1)
	{
		...
		line = io_decode_ulong(line, 10, &number_of_nodes_on_elements);
		line = io_decode_ulong(line, 10, &number_data_values_per_nodes);

		/* Gestion du code d'expansion */
		if(data_expansion_code == 2)
			nb = number_data_values_per_nodes; /* Données identiques pour tous les points */
		else
			nb = number_data_values_per_nodes * number_of_nodes_on_elements; /* Données différenciées pour tous les points */

		/* On sauvegarde les data dans le container */
		value = io_analysis_value_add(analysis, nb, number);
		value->nb_values = nb;
		...
		/***** L+ *****/
		status = parser_UNV57_read_values(value, file);
	}
 ...
 */
#if 0
static int parser_UNV57_write_data(const io_analysis_value_t *value,
				   io_file_t *file)
{
	char *line;
	unsigned long j;
			      ...
			      io_value_t *arg)
{
	...
	status = parser_UNV57_write_header(block, file, arg);
	if (status)
		return(status);
	...
	for(i = 0; i < nb; i += 1)
	{
		...
		/***** L+ *****/
		status = parser_UNV57_write_data(value, file);
	}
